...
non-default directory to store pidfiles (defaults to C<~/.keychain>). When this
option is used, the script does not automatically append C</.keychain> to the
...
tell keychain to scour C<~/.ssh/config> for all private keys referenced
...
This option allows you to use another directory besides C<$HOME/.keychain>.
...
each key specified on the command-line must have a prefix to explictly.
