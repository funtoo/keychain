...
=head1 SSH-AGENT DETECTION AND STARTUP ALGORITHM
...
=item 4. Relevant B<command-line options>, which include B<--ssh-allow-gpg>, 
    B<--ssh-use-gpg>, and B<--ssh-allow-forwarded>.
...
B<environment variables> defined in the current environment. If an agent is found
that is in alignment with specified B<command-line options>, then an already-running
agent is found, is considered "B<adopted>" by keychain, and the process is complete. Otherwise, we continue to the
next step.
...
the regular agent startup algorithm.
...
The algorithm has been specifically designed to prefer an ssh-agent spawned by keychain,
if that agent is currently available. This is by design, because other system software
could spawn ssh-agent and/or gpg-agent, and we want keychain to not coerced into using
these new agents -- which may be defined in the local environment -- if it already
spawned an agent that is available and valid. After all, it has likely loaded your private keys
into the agent it spawned itself, so then switching to another newly-spawned agent could result
re-prompted the user for passphrasess, or could prevent cron jobs from working due to finding
an agent that is not populated with keys.
