...
Keychain also supports gpg-agent.  It does not track keys since that
ability is not available in current versions of gpg-agent.  It simply
will start gpg-agent and make it available to following logins.
