...
S<keychain [ -hklQqV ]> [ --clear --confhost --confallhosts --debug --gpg2 --gpg-zap
--help --ignore-missing --list --noask --nocolor --nogui --noinherit --nolock --quick
--quiet --ssh-allow-forwarded --ssh-rm --ssh-spawn-gpg --ssh-zap --systemd --version
S<[ --ssh-agent-socket I<path> ] S<[ --dir I<dirname> ] [ --host I<name> ] 
[ --lockwait I<seconds> ]> S<[ --stop I<which> ] [ --timeout I<minutes> ] [ keys... ]>
...
To perform the sole action of wiping all cached keys, use the C<--ssh-wipe>
and C<--gpg-wipe> options. To remove an individual cached SSH private
...
Only perform the single action of wiping gpg-agent's cache of keys/
passphrases, and then exiting. Also see B<--ssh-wipe>.
...
Only perform the single action of wiping B<all> cached keys from the running ssh-agent,
and exit. Also see B<--ssh-rm>, B<--gpg-wipe>.
